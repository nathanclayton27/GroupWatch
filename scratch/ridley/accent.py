#!/usr/bin/env python3
"""Pick the Ridley Scott accent pair, and say what it was measured against.

    PYTHONIOENCODING=utf-8 python scratch/ridley/accent.py

Two passes, the same shape as scratch/wesanderson/accent.py:

  1. Name the colours anyone would reach for first for this film-maker and
     print, for each, the nearest already-shipped accent in CIE76. "The Blade
     Runner amber is taken" has to be a measurement, not a shrug — and the
     catalogue already ships an Alien & Predator list whose accent is a cold
     green, which is the exact corner a Ridley Scott list would wander into.
  2. Scan the whole in-band grid for the pair whose worst-case distance to
     every shipped accent is largest, then re-scan restricted to each hue
     family he is associated with, so the choice is between a colour that is
     his and a colour that is merely free.

Bands are measured off the shipped set — the L* and chroma the light accents
and the dark accents actually occupy — so the winner is a usable accent and
not a neon sitting in an empty corner.
"""
import colorsys
import json
import math
...
SIGNATURE = [
    ("#F0821E", "the Blade Runner street amber"),
    ("#C25A18", "the Blade Runner sun through smog"),
    ("#1B6E8C", "the Blade Runner spinner blue"),
    ("#2E7D6B", "the Nostromo green in Alien"),
    ("#1E3A2E", "the cold industrial green (already Alien & Predator's)"),
    ("#B08A3E", "Gladiator's dust and bronze"),
    ("#7B1E1E", "the Gladiator arena red"),
    ("#3D3B6B", "Legend's storybook indigo"),
    ("#8A5A2B", "Napoleon's mud and leather"),
]
FAMILIES = [("amber", 25, 45), ("orange-red", 10, 25), ("teal", 170, 200),
            ("indigo", 240, 270), ("bronze", 35, 55), ("brick", 0, 12)]
...
def main():
    ...
    # once this list has shipped it is in the manifest, and every measurement
    # below would find itself at distance zero
    manifest = [m for m in manifest if m["slug"] != "ridley-scott"]
    ...
    for hx, why in SIGNATURE:
        ...
        print("  %s  h=%3.0f  %-52s" % (hx, hue(hx), why))
        for d, c, s in near:
    ...
    print("\n--- 3. best in-band pair inside each family he is associated with")
    for name, lo, hi in FAMILIES:
        p = pairs(scored(Ls, lband, (lo, hi)), scored(Ds, dband, (lo, hi)))
        print("  %-10s (h %3d-%3d):" % (name, lo, hi))
...
# What shipped. Every obvious pick collides: the Blade Runner street amber
# lands 10.6 from Ghost in the Shell's accent and its smoggier version 5.9
# from Dragon Ball's, the spinner blue 3.0 from The Office's, the Nostromo
# green 2.3 from One Pace's, and the cold industrial green IS Alien &
# Predator's, which is the list this one shares three films with. The whole
# amber-to-bronze corner of the wheel is the catalogue's most crowded.
#
# This burnt-terracotta pair is the free spot inside his own palette — the
# smog-and-dust register rather than the neon one — at 16.6 worst-case CIE76
# against 18.3 for the freest pair anywhere on the wheel, which is a magenta
# with nothing to do with him. Nearest neighbours: Robin Williams' #8A4A2E
# (16.6) for the light, Kurosawa's #D08A80 (16.8) for the dark.
CHOSEN = ("#A67159", "#F2B8A6")
